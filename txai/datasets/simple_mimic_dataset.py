...
from txai.utils import upsample_dataset, preprocess_train_data, preprocess_test_data


class SimpleMimicDataset(Dataset):
    __create_key = object()

    @classmethod
    def create_datasets(
        cls,
        label_name="died_30_days",
        resample=False,
        target_positive_share=0.5,
        resampling_seed=42,
        split_seed=43,
        test_size=0.2,
    ):
        train_dataset = SimpleMimicDataset(
            cls.__create_key,
        ...
        test_dataset = SimpleMimicDataset(
            cls.__create_key,
            label_name=label_name,
            resample=resample,
            target_positive_share=target_positive_share,
    ...
    def __init__(
        ...
        if create_key != SimpleMimicDataset.__create_key:
            raise ValueError(
        ...
        try:
            data_df = pd.read_csv(os.path.join(data_dir_path, "simple_mimic.csv"))
        except FileNotFoundError:
            raise FileNotFoundError(
                f"MIMIC data file not found at identified data directory {data_dir_path}"
            )
        ...
        # Resample the train dataset if requested
        # We resample the dataset before scaling, as this was shown to
        # result in a more stable training in practice
        if train and resample:
            x_train[label_name] = y_train
            x_train[label_name] = x_train[label_name] > 0.5
            x_train = upsample_dataset(
                x_train,
                label_name,
                target_positive_share=target_positive_share,
        ...
        x_train_processed, preprocessor = preprocess_train_data(
            x_train, categorical_features=["admission_type", "gender"]
        )
