...
def run_openxai_experiment(dataset):
    ...
    basic_experimenter = GenericExperimenter(
        experiment_name=f"{dataset.capitalize()} (Basic Metrics)",
        seeds=list(range(42, 42 + 1)),
        num_epochs=100,
        loss=nn.CrossEntropyLoss,
        model_constructor=model_constructor,
    ...
    basic_experimenter.run_experiment(
        exp_methods=all_exp_methods, metrics=limited_metrics, test_dl=loader_test
    )

    basic_experimenter.report_experiment_results(run_stat_tests=False)
